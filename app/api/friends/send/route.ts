...
export async function POST(req: Request) {
  ...
  const other = userId
    ...
    : await prisma.user.findFirst({ where: { OR: [{ email: target }, { username: target }] } });
  if (!other || other.id === me!.id) return NextResponse.json({ error: "Invalid target" }, { status: 400 });
  const existing = await prisma.friendship.findFirst({ where: { OR: [ { userId: me!.id, friendId: other.id }, { userId: other.id, friendId: me!.id } ] } });
