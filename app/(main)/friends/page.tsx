          ...
          {pending?.requests?.map((r: any) => (
            <li key={r.id} className="border rounded p-2 flex justify-between">
              <span className="flex items-center gap-2">
                <span className={`inline-block w-2 h-2 rounded-full ${isOnline((r.userId===r.friendId?r.user:r.friend)?.lastSeen)?"bg-green-500":"bg-gray-400"}`} />
                {r.userId === r.friendId ? r.user.displayName : (r.userId ? r.user.displayName : r.friend.displayName)}
              </span>
          ...
          {list?.friends?.map((f: any) => {
            ...
            return (
              <li key={f.id} className="border rounded p-2 flex justify-between">
                <span className="flex items-center gap-2">
                  <span className={`inline-block w-2 h-2 rounded-full ${isOnline(other.lastSeen)?"bg-green-500":"bg-gray-400"}`} />
...
function isOnline(lastSeen?: string | Date | null) {
